...
def load_financial_records(user_id: str, as_of: date) -> dict:
    ...
    positions = fetch_all(
        lambda: supabase.table("wealth_positions")
        .select("*")
        .eq("user_id", user_id)
        .order("id")
    )
    snapshots = fetch_all(
        ...
        .select("*")
        .eq("user_id", user_id)
        .lte("value_date", as_of.isoformat())
        .order("id")
    )
    contributions = fetch_all(
        lambda: supabase.table("wealth_contributions")
        .select("*")
        .eq("user_id", user_id)
        .lte("contribution_date", as_of.isoformat())
        .order("id")
    )
