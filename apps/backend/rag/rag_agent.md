...
**Why separate annual-figures.json from the vector store?**
CPF retirement sums, OW ceiling, SRS limits change at Budget. These are
numerical figures where hallucination is unacceptable. They live in JSON,
injected directly into the system prompt as ground truth. The vector store
handles qualitative policy questions.
...
**Why LLM-based chunking instead of fixed-size chunking?**
Each chunk gets a headline, summary, and original text generated by the LLM.
This produces semantically coherent chunks rather than arbitrary splits,
significantly improving retrieval precision for policy documents.

**Retrieval strategy (answer.py):**
1. Rewrite user query for better KB retrieval
2. Embed both original and rewritten query
3. Retrieve top-K chunks for each, merge deduplicated
4. LLM rerank merged results
5. Take top FINAL_K chunks into context
6. Generate answer with injected annual figures + retrieved chunks

### RAG retrieval constants (answer.py)

```python
RETRIEVAL_K = 20   # chunks retrieved per query variant
FINAL_K = 10       # chunks passed to final answer LLM
```
