-- Stop truncating stitched chunk content in hybrid retrieval.
--
-- The previous function cut the primary chunk to 1,150 characters and the
-- related sibling to 550 whenever a source had more than one candidate. With
-- 1,500-character chunks plus a generated headline and summary, that dropped
-- the tail of the best chunk (where table rows and figures often sit). The
-- primary chunk is now returned whole (cap 2,200) and the sibling gets 900.
-- The signature and grants are unchanged.
    ...
    select
      ...
      pg_catalog.string_agg(
        pg_catalog.left(
          ...
          case when deduplicated.source_rank = 1 then 2200 else 900 end
        ),
        E'\n\n--- Related chunk ---\n\n'
...
comment on function public.hybrid_match_rag_chunks(
  ...
  double precision,
  integer
) is 'Private service-role RAG retrieval using english full text search, reciprocal rank fusion, fusion diagnostics, and whole-chunk stitched context.';
