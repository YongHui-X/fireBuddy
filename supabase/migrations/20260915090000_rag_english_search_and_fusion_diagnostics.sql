-- Stem keyword search with the english dictionary and expose fusion diagnostics.
--
-- 1. search_vector: headline, topic and content now use the english
--    configuration so "contributions" matches "contribute" and "reliefs"
--    matches "relief". source_title keeps the simple configuration so agency
--    and product names stay exact.
-- 2. hybrid_match_rag_chunks: returns fused_score and signal_count so the
--    backend can gate refusals on the fused ranking instead of raw cosine.

alter table public.rag_chunks drop column if exists search_vector;
...
comment on function public.hybrid_match_rag_chunks(
  ...
  double precision,
  integer
) is 'Private service-role RAG retrieval using english full text search, reciprocal rank fusion, and fusion diagnostics.';
